...
var distPath = path.join(__dirname, '..', 'dist', 'bootstrap-' + determineArchitecture() + '-dist');
...
if (process.env.EDGE_NATIVE) {
    ...
    edgeNative = path.resolve(__dirname, 'native', process.platform, process.arch, determineVersion(), (process.env.EDGE_USE_CORECLR ? 'edge_coreclr.node' : 'edge_nativeclr.node'));
}
else {
...
if (process.env.EDGE_USE_CORECLR && !process.env.EDGE_BOOTSTRAP_DIR && fs.existsSync(path.join(distPath, 'bootstrap.dll'))) {
    process.env.EDGE_BOOTSTRAP_DIR = path.join(distPath);
}
...
exports.func = function(language, options) {
    ...
    if (typeof language !== 'string') {
        ...
        if (process.env.EDGE_USE_CORECLR) {
            var defaultManifest = path.join(distPath, 'bootstrap.deps.json');
