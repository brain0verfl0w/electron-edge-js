...
process.chdir(path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp2.2'));
var fs = require('fs')
...
if (process.env.EDGE_USE_CORECLR && !process.env.EDGE_BOOTSTRAP_DIR && fs.existsSync(path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp2.2', 'bootstrap.dll'))) {
    process.env.EDGE_BOOTSTRAP_DIR = path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp2.2');
}
...
exports.func = function(language, options) {
    ...
    if (typeof language !== 'string') {
        ...
        if (process.env.EDGE_USE_CORECLR) {
            var defaultManifest = path.join(__dirname, 'bootstrap', 'bin', 'Release', 'netcoreapp2.2', 'bootstrap.deps.json');
